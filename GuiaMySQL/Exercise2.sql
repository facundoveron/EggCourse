...
SELECT nombre, cargo_emp FROM empleados AS Nombre;
...
del empleado.*/
SELECT nombre ,SUM(sal_emp + comision_emp) AS total_a_pagar FROM empleados WHERE id_depto = 3000 GROUP BY sal_emp ORDER BY nombre ASC;
...
SELECT sal_emp, comision_emp, SUM(sal_emp + comision_emp) AS salario_total, nombre FROM empleados WHERE comision_emp > 1000 GROUP BY sal_emp;
/*14. Obtener un listado similar al anterior, pero de aquellos empleados que NO
tienen comisión.*/
SELECT sal_emp, comision_emp, SUM(sal_emp + comision_emp) AS salario_total, nombre FROM empleados WHERE comision_emp <= 0 GROUP BY sal_emp;
/*15. Obtener la lista de los empleados que ganan una comisión superior a su sueldo.*/
SELECT * FROM empleados WHERE comision_emp > sal_emp;
/*16. Listar los empleados cuya comisión es menor o igual que el 30% de su sueldo.*/
SELECT * FROM empleados WHERE comision_emp <= (sal_emp * 3/10);
/*17. Hallar los empleados cuyo nombre no contiene la cadena “MA”*/
SELECT * FROM empleados WHERE !(nombre LIKE '%MA' OR nombre LIKE 'MA%');
...
SELECT id_emp ,nombre FROM empleados WHERE cargo_emp LIKE '%jefe%' HAVING COUNT(nombre) > 1;
/*26. Hallar los departamentos que no tienen empleados
Consulta con Subconsulta*/
SELECT id_emp ,nombre FROM empleados WHERE cargo_emp LIKE '%jefe%' HAVING COUNT(nombre) <= 0;
